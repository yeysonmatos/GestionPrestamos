...
export async function GET(request: NextRequest) {
  ...
  const header = 'Persona,Monto,Interés,Descripción,Fecha,Estado,Total Pagado,Pendiente\n'
  const rows = (loans || []).map(l => {
    const totalPaid = (l.payments || []).reduce((s: number, p: any) => s + Number(p.amount), 0)
    const remaining = Number(l.amount) - totalPaid
    return `"${l.person?.name || 'Sin nombre'}",${l.amount},${l.interest_rate}%,"${l.description || ''}",${l.date},${l.status},${totalPaid},${remaining}`
  }).join('\n')
