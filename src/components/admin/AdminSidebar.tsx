...
const menuItems = [
  ...
  { href: '/admin/payments', label: 'Pagos', icon: HandCoins },
  { href: '/admin/payment-config', label: 'Datos de pago', icon: BuildingOffice },
...
export default function AdminSidebar() {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-[#E3E9F4] border-r border-border">
      {/* Logo */}
      <div className="p-5 border-b border-border">
        <Link href="/admin" className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl overflow-hidden shrink-0">
            <img src="/gp-icon.png" alt="GP" className="w-9 h-9 object-cover" />
          </div>
          <span className="font-semibold text-foreground text-lg tracking-tight">GP Admin</span>
        </Link>
      </div>
      ...
      <div className="p-3 border-t border-border">
        ...
        <Link
          href="/dashboard"
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors w-full min-h-11 mt-0.5"
        >
  ...
  return (
    <>
      {/* Mobile header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-[#E3E9F4] border-b border-border px-4 h-14 flex items-center justify-between">
        <Link href="/admin" className="flex items-center gap-2">
          <img src="/gp-icon.png" alt="GP" className="w-8 h-8 rounded-lg shrink-0 object-cover" />
          <span className="font-semibold text-foreground">GP Admin</span>
        </Link>
